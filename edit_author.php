...
// get author id
$author_id = filter_input(INPUT_GET, 'auth', FILTER_VALIDATE_INT) ?? filter_input(INPUT_POST, 'author_id', FILTER_VALIDATE_INT);

// validate author id
if (!$author_id) {
    redirect_user(lang("redirect_auth_message"), 5, "authors.php");
}

// validate post param
if (
    filter_input(INPUT_POST, 'author_id', FILTER_VALIDATE_INT) &&
    filter_input(INPUT_POST, 'author_img', FILTER_SANITIZE_SPECIAL_CHARS) &&
    filter_input(INPUT_POST, 'author_name', FILTER_SANITIZE_SPECIAL_CHARS) &&
    filter_input(INPUT_POST, 'author_nationality', FILTER_SANITIZE_SPECIAL_CHARS) &&
    filter_input(INPUT_POST, 'author_profession', FILTER_SANITIZE_SPECIAL_CHARS) &&
    filter_input(INPUT_POST, 'author_birthday', FILTER_SANITIZE_SPECIAL_CHARS) &&
    ...
    $author_img = filter_input(INPUT_POST, 'author_img', FILTER_SANITIZE_SPECIAL_CHARS);
    $author_name = filter_input(INPUT_POST, 'author_name', FILTER_SANITIZE_SPECIAL_CHARS);
    $nationality = filter_input(INPUT_POST, 'author_nationality', FILTER_SANITIZE_SPECIAL_CHARS);
    $profession = filter_input(INPUT_POST, 'author_profession', FILTER_SANITIZE_SPECIAL_CHARS);
    $birthday = filter_input(INPUT_POST, 'author_birthday', FILTER_SANITIZE_SPECIAL_CHARS);
    $deathday = filter_input(INPUT_POST, 'author_deathday', FILTER_SANITIZE_SPECIAL_CHARS);
    ...
    if (
        !empty($deathday) &&
        strtotime($deathday) <= strtotime($birthday)
    ) {
        echo "<br><br><br><br><br>";
    ...
    if ($uploadImgStatus != 0) {
        ...
        } elseif ($uploadImgStatus == 1 && $author_img != 'auth_temp.svg') {
            unlink($upload_dir . $author_img);
        }

        // check deathday
        if (empty($deathday)) {
            $sql = UPDATE_AUTHOR_NULL;
            $value = [$author_profile, $author_name, $author_description, $profession, $nationality, $birthday, $author_id];
        } else {
            $sql = UPDATE_AUTHOR;
            $value = [$author_profile, $author_name, $author_description, $profession, $nationality, $birthday, $deathday, $author_id];
        }

        //update data to DB
        $update = insert_data($sql, $value);

        //check if data updated
        if ($update) {
            $update_status = 1;
            $statusMsg = lang('edit_author_success');
        } else {
            $update_status = 0;
            $statusMsg = lang('edit_author_fail');
        }
    }
...
// to change color message
// if insert success
if ($update_status == 1) {
    $msgContainer = "alert-success";
}
// if insert or upload fail
else if ($uploadImgStatus == 0 || $update_status == 0) {
    $msgContainer = "alert-danger";
}

//to get data form database
$results = $_SERVER['REQUEST_METHOD'] == 'POST' ? $_POST : get_data(SELECT_AUTHOR_INFO, [$author_id]);
?>
<br><br><br><br>
...
<div class="container rounded card shadow pb-5 mb-5" id="add_author">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="row px-5">
            ...
            <!-- author photo -->
            <div class="col-md-4 text-sm-center mt-4">
                <label for="author_profile" class="profile-upload">
                    <div class="profile-container">
                        <img src="upload/authors/<?= $results['author_img'] ?>" alt="author image" id="profile_img">
                    </div>
            ...
            <!-- to send author id and image need for update process -->
            <input type="hidden" name="author_id" value="<?= $results['author_id'] ?>">
            <input type="hidden" name="author_img" value="<?= $results['author_img'] ?>">

            <div class="col-md-8 text-md-end text-sm-center">

                <div class="mt-3 text-end">
                    <label for="author_name" class="blue-text mb-2 me-4"><?= lang('author_name'); ?></label>
                    <input required class="form-control" type="text" id="author_name" name="author_name" onkeyup="checkName('check_author.php', {author_name: $(this).val()}, $(this), '<?= $results['author_name'] ?>')" value="<?= $results['author_name'] ?>" placeholder="<?php echo lang('author_name_place') ?>" autofocus />
                    <div class="invalid-feedback"></div>
                </div>
                ...
                <div class="row">
                    <div class="col-6">
                        <div class="mt-3 text-end">
                            ...
                            <input required class="form-control" type="text" id="nationality" name="author_nationality" value="<?= $results['author_nationality'] ?>" placeholder="<?php echo lang('nationality_place') ?>" />
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="mt-3 text-end">
                            <label for="profession" class="blue-text mb-2 me-4"><?= lang('profession'); ?></label>
                            <input required class="form-control" type="text" id="profession" name="author_profession" value="<?= $results['author_profession'] ?>" placeholder="<?php echo lang('profession_place') ?>" />
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-6">
                        <div class="mt-3 text-end">
                            ...
                            <input required class="form-control" type="date" id="birthday" onchange="checkDate('#birthday','#deathday')" name="author_birthday" value="<?= $results['author_birthday'] ?>" />
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="mt-3 text-end">
                            ...
                            <input class="form-control" type="date" id="deathday" onchange="checkDate('#birthday','#deathday')" name="author_deathday" value="<?= $results['author_deathday'] ?>" />
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-3 text-end">
                    ...
                    <textarea required class="form-control" id="author_description" name="author_description" rows="3" placeholder="<?php echo lang('author_description_place') ?>"><?= $results['author_description'] ?></textarea>
                </div>
